...
# SOFC_Turbofan_ElectricMotor_DuctedFan.py
# 
# Created:  June 2021, Pedro Garcia Gonzalez (TU Delft)
...
## @ingroup Components-Energy-Networks
class SOFC_Turbofan_ElectricMotor_DuctedFan(Propulsor):
    """ This is a SOFC-turbofan coupled system.
    
        Assumptions:
        Air is extracted from the last stage of the HPC for the SOFC.
        SOFC byproducts are injected into the combustion chamber.
    ...
    def __defaults__(self):
        ...
        self.tag = 'SOFC_Turbofan_ElectricMotor_DuctedFan'
        self.number_turbofan    = 0.0
        self.number_ducted_fan  = 0.0
        self.fraction_air_SOFC = 0.0
        self.nacelle_diameter_turbofan     = 0.0
        self.engine_length_turbofan        = 0.0
        self.bypass_ratio_turbofan         = 0.0
        self.nacelle_diameter_ducted_fan = 0.0
        self.engine_length_ducted_fan = 0.0
        ...
        self.generative_design_characteristics = ['sealevel_static_thrust','number_turbofan','bypass_ratio','non_dimensional_origin[0][0]','non_dimensional_origin[0][1]','non_dimensional_origin[0][2]']
        ...
        # Elements required for the network
        self.propulsor = None
        self.battery = None
    ...
    def evaluate_thrust(self,state):
        ...
        ducted_fan = self.ducted_fan
        
        #General considerations
        thrust_turbofan                    = self.thrust_turbofan
        bypass_ratio              = self.bypass_ratio
        number_turbofan         = self.number_turbofan
        number_ducted_fan = self.number_ducted_fan
        fraction_air_SOFC = self.fraction_air_SOFC
        ...
        #link the SOFC to the high pressure compressor
        SOFC.inputs.stagnation_temperature = high_pressure_compressor.outputs.stagnation_temperature
        SOFC.inputs.stagnation_temperature = high_pressure_compressor.outputs.stagnation_pressure

        #Flow through the SOFC
        SOFC(conditions)

        #SOFC outputs


        #link the combustor to the high pressure compressor and SOFC
        combustor.inputs.stagnation_temperature                = high_pressure_compressor.outputs.stagnation_temperature*(1-fraction_air_SOFC)+SOFC.outputs.stagnation_temperature*fraction_air_SOFC
        combustor.inputs.stagnation_pressure                   = high_pressure_compressor.outputs.stagnation_pressure*(1-fraction_air_SOFC)+SOFC.outputs.stagnation_pressure*fraction_air_SOFC
        ...
        thrust.inputs.number_turbofan                          = number_turbofan
    ...
    def engine_out(self,state):
        ...
        results.thrust_force_vector = results.thrust_force_vector/self.number_turbofan*(self.number_turbofan-1)
        results.vehicle_mass_rate   = results.vehicle_mass_rate/self.number_turbofan*(self.number_turbofan-1)
